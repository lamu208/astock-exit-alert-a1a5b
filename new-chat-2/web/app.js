...
function renderDetail(stock) {
  ...
  const priceDigits = instrumentDigits(stock); $('#detail-price').textContent = unavailable ? '—' : formatPrice(quote.price, stock); const changeNode = $('#detail-change'); changeNode.textContent = unavailable ? '—' : formatChange(change); changeNode.className = trendClass(change); const actionNode = $('#detail-action'); actionNode.textContent = unavailable ? '待连接' : actionText[action]; actionNode.className = action; $('#detail-source').textContent = unavailable ? `行情未连接：${quote.error || '数据源暂不可用'}` : `数据来源：${quote.source || state.data.data_source || 'Vibe-Trading'} · 历史 K 线 ${quote.history_bars || 0} 根（${quote.history_status === 'complete' ? '完整' : quote.history_status === 'partial' ? '部分' : '不足'}）`;
  $('#detail-metrics').innerHTML = unavailable ? `<div class="metric-item"><span>连接提示</span><strong class="danger">${escapeHtml(quote.error || '数据源暂不可用')}</strong></div>` : `<div class="metric-item"><span>MA5（趋势线）</span><strong class="${number(quote.price) < number(quote.ma5 || quote.trend_line) ? 'danger' : ''}">${formatMetric(quote.ma5 || quote.trend_line, priceDigits)}</strong></div><div class="metric-item"><span>MA10</span><strong>${formatMetric(quote.ma10, priceDigits)}</strong></div><div class="metric-item"><span>MA20</span><strong>${formatMetric(quote.ma20, priceDigits)}</strong></div><div class="metric-item"><span>MA60</span><strong>${formatMetric(quote.ma60, priceDigits)}</strong></div><div class="metric-item"><span>量比（vs 5日均量）</span><strong class="${number(quote.volume_ratio) >= 1.3 ? 'warning' : ''}">${quote.avg_volume_5 ? `${number(quote.volume_ratio || quote.volume / quote.avg_volume_5).toFixed(2)}×` : '暂无数据'}</strong></div><div class="metric-item"><span>成交额</span><strong>${formatNumber(quote.amount)}</strong></div><div class="metric-item"><span>今开 / 昨收</span><strong>${formatMetric(quote.open, priceDigits)} / ${formatMetric(quote.prev_close, priceDigits)}</strong></div><div class="metric-item"><span>最高 / 最低</span><strong>${formatMetric(quote.high, priceDigits)} / ${formatMetric(quote.low, priceDigits)}</strong></div>`;
