...
function renderWatchlist() {
  ...
  host.innerHTML = stocks.map((stock) => {
    ...
    return `<button class="stock-row" type="button" data-symbol="${escapeHtml(stockKey(stock))}"><div class="stock-title"><strong class="stock-name">${escapeHtml(stock.name || quote.name || stock.symbol)}</strong><span class="stock-meta"><b class="stock-code-badge">${escapeHtml(stockCode(stock))}</b><i class="live-mark">${unavailable ? '⚠' : '▣'}</i></span></div><div class="stock-middle"><div class="spark-wrap">${sparkline(quote.recent_closes || [], change)}</div><div class="stock-quote"><span class="stock-percent ${tone}">${unavailable ? '—' : formatChange(change)}</span><span class="stock-price ${tone}">${unavailable ? '行情不可用' : number(quote.price).toFixed(2)}</span></div></div><div class="stock-decision"><b class="decision-tag ${unavailable ? 'none' : action}">${displayDecision}</b></div></button>`;
  }).join('');
