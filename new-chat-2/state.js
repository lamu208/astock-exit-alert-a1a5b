...
function exchangeSymbol(value) {
  ...
  if (code.startsWith("16")) return `${code}.SZ`;
...
function vendorKey(symbol) {
  ...
  if (code.startsWith("16")) return `sz${code}`;
...
async function fetchQuotes(symbols) {
  ...
  try {
    ...
    result = parseQuote(raw);
  } catch {}
  await Promise.all(symbols.map(async (symbol) => {
    const code = digits(symbol);
    if (!code || result[code]) return;
    try {
      const response = await fetch(`https://push2.eastmoney.com/api/qt/stock/get?secid=${eastmoneySecId(symbol)}&fields=f43,f44,f45,f46,f47,f48,f57,f58,f60,f170`, { headers: { "user-agent": "A-share-exit-alert/1.0" } });
...
async function fetchHistory(symbol) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const rows = normalizeHistoryRows(entry.qfqday || entry.day || entry.qfqweek || []);
  ...
  try {
    ...
    return normalizeHistoryRows((payload?.data?.klines || []).map((line) => String(line).split(",")));
  } catch {
    return [];
  }
}
