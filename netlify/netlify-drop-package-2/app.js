...
function renderDetail(stock) {
  ...
  $('#detail-metrics').innerHTML = unavailable ? `<div class="metric-item"><span>连接提示</span><strong class="danger">${escapeHtml(quote.error || '数据源暂不可用')}</strong></div>` : `<div class="metric-item"><span>MA5（趋势线）</span><strong class="${number(quote.price) < number(quote.ma5 || quote.trend_line) ? 'danger' : ''}">${number(quote.ma5 || quote.trend_line).toFixed(2)}</strong></div><div class="metric-item"><span>MA10</span><strong>${number(quote.ma10).toFixed(2)}</strong></div><div class="metric-item"><span>MA20</span><strong>${number(quote.ma20).toFixed(2)}</strong></div><div class="metric-item"><span>MA60</span><strong>${number(quote.ma60).toFixed(2)}</strong></div><div class="metric-item"><span>量比（vs 5日均量）</span><strong class="${number(quote.volume_ratio) >= 1.3 ? 'warning' : ''}">${quote.avg_volume_5 ? `${number(quote.volume_ratio || quote.volume / quote.avg_volume_5).toFixed(2)}×` : '数据不足'}</strong></div><div class="metric-item"><span>成交额</span><strong>${formatNumber(quote.amount)}</strong></div><div class="metric-item"><span>今开 / 昨收</span><strong>${number(quote.open).toFixed(2)} / ${number(quote.prev_close).toFixed(2)}</strong></div><div class="metric-item"><span>最高 / 最低</span><strong>${number(quote.high).toFixed(2)} / ${number(quote.low).toFixed(2)}</strong></div>`;
